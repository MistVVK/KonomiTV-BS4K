...
class KonomiTVBS4KTLVRainFallbackMonitor:
    """生TLVを独立したmetadata helperへ複製し、降雨対応Videoの送出状態を継続監視する。"""
    ...
    def __init__(
        ...
        Args:
            main_service_id (int): 主階層サービスのSID。
            rain_service_id (int): 降雨対応サービスのSID。
        ...
        # contextごとに最後に受信した完全MPTのVideo有無を保持する。
    ...
    async def _readMetadata(self, stdout: asyncio.StreamReader) -> None:
        ...
        while True:
            ...
            if metadata.snapshot_type == 'MPT' and metadata.snapshot_context_id is not None:
                self._mpt_video_availability[metadata.snapshot_context_id] = (
                    metadata.snapshot_context_id in metadata.video_context_ids
                )
