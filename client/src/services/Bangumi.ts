...
export interface IBangumiProfile {
...
class Bangumi {
    ...
    static async logoutAccount(): Promise<boolean> {
        ...
        // API リクエストを実行
        const response = await APIClient.post('/bangumi/logout');
    ...
    /**
     * 共有 Bangumi 連携の公開プロフィールを取得する
     * @returns 公開プロフィール。未取得時は null
     */
    static async fetchProfile(): Promise<IBangumiProfile | null> {
        const response = await APIClient.get<IBangumiProfile>('/bangumi/me');
        if (response.type === 'error') {
            APIClient.showGenericError(response, 'Bangumi 連携状態を取得できませんでした。');
            return null;
        }
     ...
     * @param show_error 通信失敗時にトーストを出すか。再生中の定期送信では出さない
