 ...
 * dantto4k は ID3v2.4 を出力するが、録画ファイルを別ツールで再多重化した場合に備え
 * ID3v2.3 の通常整数 frame size も読み取れるようにしている。
 */
function extractPrivateFrames(data: Uint8Array): {owner: string; payload: Uint8Array}[] {
    const frames: {owner: string; payload: Uint8Array}[] = [];
    ...
    for (let tag_begin = 0; tag_begin + 10 <= data.length;) {
        ...
        // dantto4k は extended header / unsynchronisation を使わない。unsynchronisation は
        // payload 自体を書き換えるため、未解除のまま誤解析せず安全側でタグを捨てる。
...
export default class ARIBTTMLDecoder {
    ...
    public pushID3v2Data(
        ...
        for (const frame of extractPrivateFrames(data)) {
