...
AMD_WARNING_START = '<!-- AMD_RUNTIME_WARNING_START -->'
AMD_WARNING_END = '<!-- AMD_RUNTIME_WARNING_END -->'
...
def main() -> None:
    ...
    parser.add_argument('--include-amd-runtime', action='store_true')
    ...
    target = f'cuda{args.cuda_version}-{"amd" if args.include_amd_runtime else "no-amd"}'
    target_document = '\n'.join([
        '> **Docker build target**',
        '>',
        f'> - Target: `{target}`',
        f'> - CUDA package series: `{args.cuda_version}`',
        f'> - AMD proprietary runtime included: `{str(args.include_amd_runtime).lower()}`',
        '> - NVIDIA driver libraries are supplied by the host at runtime and are not included in this image.',
    ])

    # AMD runtime を含む場合は、再配布警告を読んだ直後にビルド条件を確認できるよう警告全体の直後へ配置する
    # AMD runtime を含まない場合は警告自体を除去し、文書タイトルの直後へ同じカードを配置する
    if args.include_amd_runtime:
        target_insertion_point = base_document.index(AMD_WARNING_END) + len(AMD_WARNING_END)
    else:
        warning_start = base_document.index(AMD_WARNING_START)
        warning_end = base_document.index(AMD_WARNING_END) + len(AMD_WARNING_END)
