// この一覧は放送アプリ内の絶対 URL を VFS scope へ書き換えるためだけに使う。
// 接続先のセキュリティ境界は既存 DataBroadcastingHTTPClient の公開 IP ポリシーが担う。
const KONOMITV_BS4K_ARIB_HTML5_EXTERNAL_PROXY_ALLOWED_HOSTS = [
    '4kdata-p.qvc.jp',
    'api.qvc.jp',
    'api.nhk.or.jp',
    'beacon.nhk.jp',
...
const KONOMITV_BS4K_EXTERNAL_PROXY_RUNTIME = String.raw`
  ...
  async function serveExternalProxy(request, url) {
    ...
    const proxyUrl = new URL(
      "/api/data-broadcasting/request/" + encodeURIComponent(resolved.upstreamUrl.href),
...
/** libaribhtml5 の生成済み VFS Worker に KonomiTV-BS4K の外部文書プロキシを追加する。 */
export function patchKonomiTVBS4KARIBVFSWorkerSource(source: string): string {
    const serve_marker = '  async function serve(request) {';
    if (!source.includes(serve_marker) || !source.includes(ORIGINAL_FETCH_HANDLER)) {
        throw new Error('Unsupported libaribhtml5 VFS Worker layout.');
    }

    // 放送波由来の HTML / CSS / JavaScript に埋め込まれた絶対 URL も Worker scope 内へ戻す。
    return source
        .replace(serve_marker, `${KONOMITV_BS4K_EXTERNAL_PROXY_RUNTIME}\n${serve_marker}`)
        .replace(
