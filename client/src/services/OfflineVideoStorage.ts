...
export default class OfflineVideoStorage {
    ...
    private static readonly DB_NAME = 'KonomiTV-BS4K-OfflineVideos';
    ...
    /** 指定した保存世代が、実行中ジョブまたは完成済み動画から参照されているかを返す */
    static async isGenerationReferenced(videoID: number, generationID: string): Promise<boolean> {
        const database = await this.openDatabase();
        const transaction = database.transaction([this.JOB_STORE_NAME, this.VIDEO_STORE_NAME], 'readonly');
        const [video, jobs] = await Promise.all([
            transaction.objectStore(this.VIDEO_STORE_NAME).get(videoID),
            transaction.objectStore(this.JOB_STORE_NAME).getAll(),
        ]);
        await transaction.done;

        // completeJob() はジョブ削除と動画登録を同じトランザクションで行うため、同じスナップショットで両方を検査する
        return video?.generation_id === generationID || jobs.some(job =>
            job.video_id === videoID && job.generation_id === generationID &&
            ['Waiting', 'Downloading', 'Finalizing'].includes(job.state),
        );
    }

    /** 同じ録画番組の実行中ジョブがなければ、古い終端ジョブを置き換えて新しいジョブを追加する */
    static async putJobIfVideoIdle(job: IOfflineDownloadJob): Promise<void> {
        ...
        // 再保存を開始した時点で過去の失敗表示は役目を終えるため、同じ録画の終端ジョブを残さない
        for (const existingJob of jobs) {
            if (existingJob.video_id === job.video_id) {
                await transaction.store.delete(existingJob.job_id);
            }
        }
    ...
    static async updateActiveJob(job: IOfflineDownloadJob): Promise<boolean> {
        ...
        if (latestJob === undefined || ['Failed', 'Cancelled'].includes(latestJob.state)) {
            await transaction.done;
            return false;
        }
        await transaction.store.put(job);
    ...
    static async transitionActiveJobToTerminalState(
        ...
        if (job === undefined || ['Failed', 'Cancelled'].includes(job.state)) {
            await transaction.done;
            return null;
        }
        job.state = state;
    ...
    private static isVideoCacheComplete(video: IOfflineVideo, cachedRequestURLs: string[]): boolean {
        ...
        const expectedAssetURLs = video.asset_paths.map(path => `${generationPrefix}${path}`);
        const cachedGenerationURLs = cachedRequestURLs.filter(url => url.startsWith(generationPrefix));
        const missingAssetCount = expectedAssetURLs.filter(url => cachedRequestURLs.includes(url) === false).length;
        const hasMasterPlaylist = expectedAssetURLs.includes(`${generationPrefix}playlist.m3u8`);
        const isComplete = video.asset_paths.length === video.asset_count && hasMasterPlaylist === true &&
            missingAssetCount === 0 && cachedGenerationURLs.length >= video.asset_count;

        // 応答終端で記録した全アセットのいずれかが欠けた世代だけを削除対象として報告する
        ...
        if (isComplete === false) {
            console.warn(
                ...
                `has_master_playlist: ${hasMasterPlaylist}, expected_asset_count: ${video.asset_count}, ` +
                `actual_asset_count: ${cachedGenerationURLs.length}, missing_asset_count: ${missingAssetCount}]`,
            );
        }
        return isComplete;
    }
