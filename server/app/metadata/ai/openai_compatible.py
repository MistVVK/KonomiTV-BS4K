...
def _BuildEpisodeLookupSystemPrompt(prompt_variant: AIPromptVariant) -> str:
    ...
    return (
        ...
        'Do not invent an episode number. Use season 1 when the work has no explicit seasons. '
        'Use NoPublishedNumber for a special or recap that belongs to the work but has no published number. '
        'Use NotNumbered only when the continuing program does not use episode numbering. '
        'Use InsufficientEvidence with null season_number and episode_number when evidence is weak. '
